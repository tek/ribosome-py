...
class HandlerSpec:
    ...
    def run(self, f: Callable[[Msg], R]) -> Maybe[Msg2]:
        s, res = self.validator.transit(f(Msg()))
        return k(res.head).must(be_just(have_type(Msg2)))
    ...
    def eso(self) -> Expectation:
        ...
        res = f(Msg())
        valid = self.validator.transit(res)[1].head
        res1 = valid.to_either('').flat_map(_.io.attempt)
        valid1 = res1 / self.validator.transit / _[1] // _.head
        return (
