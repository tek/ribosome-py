...
class MessageRequestHandler(Generic[M], RequestHandler1):
        ...
        if isinstance(obj, ribosome.NvimStatePlugin):
            try:
                ...
                try:
                    ...
                    self.log.caught_exception_error(f'sending {msg} from request handler {self.vim_name}', e)
        else:
...
class JsonMessageRequestHandler(Generic[M], MessageRequestHandler):
    ...
    def _extract_args(self, args: Tuple[str]):
        ...
        pos_args, data_args = (tuple(args[:self.min]), tuple(args[self.min:]))
        data = ' '.join(data_args)
...
__all__ = ('RequestHandler1', 'MessageRequestHandler', 'JsonMessageRequestHandler')
